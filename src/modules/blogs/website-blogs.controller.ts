...
export class WebsiteBlogsController {
  ...
  @ApiOperation({
    ...
    description: 'Returns a paginated list of published and active blogs that are associated with the authenticated website.',
  })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Limit items per page' })
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Search term for titles, slugs, or tags' })
  async findAll(
  ...
  @ApiOperation({
    ...
    description: 'Fetches details of a specific blog and verifies that it is active, published, and belongs to the authenticated website.',
  })
  @ApiParam({ name: 'idOrSlug', description: 'The MongoDB ID or the URL-friendly slug of the blog' })
  async findOne(@CurrentWebsite() website: any, @Param('idOrSlug') idOrSlug: string) {
    let blog: any;
    ...
    const hasWebsite = blog.websites.some(
      (w: any) => w._id?.toString() === website.id || w.id === website.id || w.toString() === website.id
    );
  ...
  @ApiOperation({
    ...
    description: 'Increments the like count for the specified blog. Verifies that the blog belongs to this website.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Increments the view count for the specified blog. Verifies that the blog belongs to this website.',
  })
  @ApiParam({ name: 'id', description: 'The MongoDB ID of the blog' })
  async incrementViews(@CurrentWebsite() website: any, @Param('id') id: string) {
    await this.validateBlogBelongsToWebsite(id, website.id);
  ...
  @ApiOperation({
    ...
    description: 'Creates a pending comment on the specified blog post. Verifies that the blog belongs to this website.',
  })
  @ApiParam({ name: 'id', description: 'The MongoDB ID of the blog' })
  async addComment(
    @CurrentWebsite() website: any,
  ...
  @ApiOperation({
    ...
    description: 'Fetches the list of approved comments for a specific blog post. Verifies that the blog belongs to this website.',
  })
  @ApiParam({ name: 'id', description: 'The MongoDB ID of the blog' })
  ...
  private async validateBlogBelongsToWebsite(blogId: string, websiteId: string) {
    ...
    const hasWebsite = blog.websites.some(
      (w: any) => w._id?.toString() === websiteId || w.id === websiteId || w.toString() === websiteId
    );
