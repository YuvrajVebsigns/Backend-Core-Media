...
export class EmailProcessor {
  ...
  async handleEventRegistration(job: Job) {
    const { email, name, eventName, passCode, startDate, endDate, location } = job.data;
    
    this.logger.debug(`[Background Job Started] Sending event registration email to ${email} for ${eventName}...`);

    const calendarUrl = this.generateGoogleCalendarUrl(eventName, startDate, endDate, location);
    ...
    this.logger.debug(`[Background Job Complete] Registration email successfully sent to ${email}!`);
  }

  private generateGoogleCalendarUrl(title: string, start: string, end: string, location: string): string {
    const format = (dateStr: string) => new Date(dateStr).toISOString().replace(/-|:|\.\d+/g, '');
